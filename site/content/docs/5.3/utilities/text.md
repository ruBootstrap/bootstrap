...
{{< callout warning >}}
Обратите внимание, что [перенос слов на арабском языке невозможен](https://rtlstyling.com/posts/rtl-styling#3.-line-break), который является наиболее часто используемым языком RTL. Поэтому `.text-break` удаляется из нашего RTL-скомпилированного CSS.
{{< /callout >}}
