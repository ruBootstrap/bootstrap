...
`.card-img-top` и `.card-img-bottom` соответственно закругляют верхний и нижний углы, чтобы они соответствовали границам карточки. С помощью `.card-text` на карту можно добавить текст. Текст внутри `.card-text` также можно стилизовать с помощью стандартных HTML-тегов.
