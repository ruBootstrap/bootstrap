---
title: Контейнеры
description: Контейнеры — это фундаментальный строительный блок Bootstrap, который содержит, дополняет и выравнивает ваш контент в пределах определенного устройства или области просмотра.
toc: true
---
...
Таблица ниже иллюстрирует, как `max-width` каждого контейнера сравнивается с исходными `.container` и `.container-fluid` в каждой контрольной точке.

Посмотрите на них в действии и сравните их в нашем [примере сетки]([[docsref:/examples/grid/#containers]]).
...
Дополнительную информацию и примеры того, как изменять наши карты и переменные Sass, см. в [разделе Sass документации Grid]([[docsref:/layout/grid#css]]).
