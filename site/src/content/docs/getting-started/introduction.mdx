...
Начните с включения готовых к производству CSS и JavaScript Bootstrap через CDN без необходимости каких-либо шагов сборки. Посмотрите на практике с помощью этой [демо Bootstrap CodePen](https://codepen.io/team/bootstrap/pen/qBamdLj).
...
3. **Привет, мир!** Откройте страницу в вашем браузере по вашему выбору, чтобы увидеть вашу страницу Bootstrapped. Теперь вы можете начать разработку с помощью Bootstrap, создав свой собственный [макет]([[docsref:/layout/grid]]), добавив десятки [компонентов]([[docsref:/components/buttons]]) и используя [наши официальные примеры]([[docsref:/examples]]).
...
Вы также можете использовать CDN для загрузки любой из наших [дополнительных сборок, перечисленных на странице «Содержание»]([[docsref:/getting-started/contents]]).

При использовании ссылок CDN обязательно используйте атрибут `integrity` для проверки правильности файлов и версий. Эти хэши уникальны для каждого файла и версии Bootstrap, поэтому при обновлении до новой версии убедитесь, что атрибут `integrity` также обновлен.

Мы также включаем атрибут `crossorigin="anonymous"` для предотвращения ошибок [CORS](https://developer.mozilla.org/en-US/docs/Web/HTTP/CORS).

## Следующие шаги

- Узнайте больше о некоторых [важных глобальных параметрах среды](#important-globals), которые использует Bootstrap.
- Узнайте о том, что включено в Bootstrap, в нашем [разделе содержимого]([[docsref:/getting-started/contents/]]) и в списке [компонентов, требующих JavaScript](#js-components) ниже.
- Нужно немного больше мощности? Рассмотрите возможность сборки с помощью Bootstrap, [включив исходные файлы через менеджер пакетов]([[docsref:/getting-started/download#package-managers]]).
- Хотите использовать Bootstrap как модуль с `<script type="module">`? Пожалуйста, обратитесь к нашему разделу [использование Bootstrap как модуля]([[docsref:/getting-started/javascript#using-bootstrap-as-a-module]]).

## JS компоненты

Любопытно, какие компоненты явно требуют наш JavaScript и Popper? Если вы совсем не уверены в общей структуре страницы, продолжайте читать, чтобы увидеть пример шаблона страницы.
...
Bootstrap использует несколько важных глобальных стилей и настроек, все из которых почти исключительно направлены на *нормализацию* кроссбраузерных стилей. Давайте углубимся.

### HTML5 doctype

Bootstrap требует использования HTML5 doctype. Без него вы увидите странную и неполную стилизацию.
...
Для улучшения кросс-браузерного рендеринга мы используем [Reboot]([[docsref:/content/reboot]]) для исправления несоответствий между браузерами и устройствами, одновременно обеспечивая немного более обоснованные сбросы для общих элементов HTML.

## Сообщество

Будьте в курсе развития Bootstrap и обращайтесь к сообществу с помощью этих полезных ресурсов.

- Читайте и подписывайтесь на [Официальный блог Bootstrap]([[config:blog]]).
...
Вы также можете подписаться на [@getbootstrap на X](https://x.com/[[config:x]]) для получения последних сплетен и потрясающих музыкальных клипов.
