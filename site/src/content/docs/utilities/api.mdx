---
title: Служебный API
description: API утилиты — это инструмент на основе Sass для генерации классов утилит.
aliases: "/docs/[[config:docs_version]]/utilities/"
toc: true
---

Утилиты Bootstrap генерируются с помощью нашего API утилит и могут использоваться для изменения или расширения нашего стандартного набора классов утилит через Sass. Наш API утилит основан на серии карт Sass и функций для генерации семейств классов с различными параметрами. Если вы не знакомы с картами Sass, прочитайте [официальную документацию Sass](https://sass-lang.com/documentation/values/maps/), чтобы начать.
...
| [`values`](#values) | **Обязательно** | – | Список значений или карта, если вы не хотите, чтобы имя класса совпадало со значением. Если `null` используется в качестве ключа карты, `class` не добавляется к имени класса. |
| [`class`](#class) | Необязательно | null | Имя сгенерированного класса. Если не указано и `property` является массивом строк, `class` по умолчанию будет первым элементом массива `property`. Если не указано и `property` является строкой, ключи `values` используются для имен `class`. |
...
| [`state`](#states) | Необязательно | null | Список вариантов псевдоклассов (например, `:hover` или `:focus`) для генерации. |
| [`responsive`](#responsive) | Необязательно | `false` | Логическое значение, указывающее, следует ли генерировать адаптивные классы. |
| `rfs` | Необязательно | `false` | Логическое значение для включения [плавного масштабирования с помощью RFS]([[docsref:/getting-started/rfs]]). |
| [`print`](#print) | Необязательно | `false` | Логическое значение, указывающее, нужно ли генерировать классы печати. ​​|
...
Требуемый ключ `property` должен быть установлен для любой утилиты, и он должен содержать допустимое свойство CSS. Это свойство используется в наборе правил сгенерированной утилиты. Если ключ `class` опущен, он также служит именем класса по умолчанию. Рассмотрим утилиту `text-decoration`:
...
В виде списка, например, [утилиты `text-decoration`]([[docsref:/utilities/text#text-decoration]]):

```scss
values: none underline line-through
```

В качестве карты, например, с помощью [утилит `opacity`]([[docsref:/utilities/opacity]]):
...
Как переменная Sass, которая задает список или карту, как в наших [утилитах `position`]([[docsref:/utilities/position]]):
...
Используйте опцию `local-vars`, чтобы указать карту Sass, которая будет генерировать локальные переменные CSS в наборе правил класса утилиты. Обратите внимание, что может потребоваться дополнительная работа для использования этих локальных переменных CSS в сгенерированных правилах CSS. Например, рассмотрим наши утилиты `.bg-*`:
...
Используйте опцию `state` для генерации вариаций псевдоклассов. Примерами псевдоклассов являются `:hover` и `:focus`. Когда предоставляется список состояний, для этого псевдокласса создаются имена классов. Например, чтобы изменить непрозрачность при наведении, добавьте `state: hover`, и вы получите `.opacity-hover:hover` в скомпилированном CSS.
...
Добавьте логическое значение `responsive` для генерации адаптивных утилит (например, `.opacity-md-25`) для [всех контрольных точек]([[docsref:/layout/breakpoints]]).
...
Теперь, когда вы знакомы с тем, как работает API утилит, узнайте, как добавлять собственные классы и изменять наши утилиты по умолчанию.
...
Новые утилиты можно добавить в карту `$utilities` по умолчанию с помощью `map-merge`. Убедитесь, что наши необходимые файлы Sass и `_utilities.scss` импортированы первыми, затем используйте `map-merge` для добавления дополнительных утилит. Например, вот как добавить адаптивную утилиту `cursor` с тремя значениями.
...
Измените существующие утилиты в карте `$utilities` по умолчанию с помощью функций `map-get` и `map-merge`. В примере ниже мы добавляем дополнительное значение к утилитам `width`. Начните с начальной `map-merge`, а затем укажите, какую утилиту вы хотите изменить. Оттуда извлеките вложенную карту `"width"` с помощью `map-get`, чтобы получить доступ и изменить параметры и значения утилиты.
...
Вы можете добавлять, удалять и изменять множество утилит одновременно с помощью функции Sass [`map-merge()`](https://sass-lang.com/documentation/modules/map/#merge). Вот как можно объединить предыдущие примеры в одну большую карту.
...
Это не выводит ничего в формате RTL благодаря [директиве управления RTLCSS `remove`](https://rtlcss.com/learn/usage-guide/control-directives/#remove).
